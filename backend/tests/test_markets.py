...
def test_registry_self_consistency():
    ...
    crypto = get_market(MARKET_CRYPTO)
    assert crypto.has_limit is False
    assert crypto.default_limit_pct is None
    assert crypto.stamp_tax == 0.0
    assert crypto.stamp_tax_double_sided is False
    assert crypto.t_plus == 0
    assert crypto.lot_size == 1
    assert crypto.exchanges == ("BINANCE",)
...
class TestCryptoMarket:
    @pytest.mark.parametrize(
        ...
        [
            ("BTCUSDT.CRYPTO", MARKET_CRYPTO),
            ("ETHUSDT.CRYPTO", MARKET_CRYPTO),
            ("btcusdt.crypto", MARKET_CRYPTO),
            (" BTCUSDT.CRYPTO ", MARKET_CRYPTO),
        ],
    )
    def test_market_of_crypto_suffix(self, symbol, expected):
        # .CRYPTO 后缀必须先注册，否则 BTCUSDT 会被无后缀字母兜底误判为美股
        assert market_of(symbol) == expected
    ...
    @pytest.mark.parametrize("raw", ["BTC-USDT", "BTCUSDT.CRYPTO"])
    def test_normalize_then_market_of_is_crypto(self, raw):
        assert market_of(normalize_symbol(raw)) == MARKET_CRYPTO

    def test_crypto_has_no_limit(self):
        assert market_limit_pct("BTCUSDT.CRYPTO") is None
        assert has_limit(MARKET_CRYPTO) is False

    def test_crypto_fees_and_units(self):
        assert stamp_tax_for(MARKET_CRYPTO) == 0.0
        assert stamp_tax_double_sided(MARKET_CRYPTO) is False
        assert lot_size_for(MARKET_CRYPTO) == 1
        assert exchanges_for(MARKET_CRYPTO) == ["BINANCE"]

    @pytest.mark.parametrize("hour", [0, 3, 9, 12, 20, 23])
    def test_crypto_trades_7x24(self, hour):
        # 7x24 市场任意时刻都在交易时段内（sessions 近似全天）
        assert is_trading_now(MARKET_CRYPTO, _bj(hour, 30)) is True
