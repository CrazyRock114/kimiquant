...
class StrategyBacktestRequest(BaseModel):
    ...
    market: str = "cn"   # cn | hk | us | crypto（多市场扩展；决定默认费率，涨跌停规则随数据自动）
...
def strategy_run(req: StrategyBacktestRequest, request: Request):
    ...
    market 参数（cn/hk/us/crypto）：未显式传印花税时按市场取默认（港股双边 0.1%、
    A 股卖出 0.05%、美股与 crypto 无）；涨跌停买入/卖出拦截由 enriched 信号列驱动，
    港美股/crypto 无 signal_limit_* 列 → 自动不拦截。
    """
...
async def strategy_stream(
    ...
    async def event_generator():
        ...
        if is_new and not job.done:
            default_stamp, default_double_sided = _market_default_fees(market)
            cfg = StrategyBacktestConfig(
