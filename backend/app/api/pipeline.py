...
async def run_market_now(request: Request, market: str) -> dict:
    """异步触发港美股/crypto 数据同步 + enriched 计算（多市场扩展）。

    market: hk | us | crypto。A 股仍走 POST /api/pipeline/run。
    立即返回 job_id，客户端轮询 /jobs/{id} 拿进度。
    """
    from app.markets import ALL_MARKETS

    if market not in ("hk", "us", "crypto"):
        raise HTTPException(status_code=400, detail=f"market 必须为 hk/us/crypto（可选: {ALL_MARKETS}）")
