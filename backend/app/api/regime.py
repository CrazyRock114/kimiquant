...
def regime_history(
    ...
    """历史环境时序(含状态/指标)。默认最近 N 天。港美股/crypto 读独立时序表。"""
    global _cache, _cache_ts
...
@router.get("/latest")
def regime_latest(request: Request, market: str = Query("cn", description="cn|hk|us|crypto")):
    """最新一日环境(轻量)。"""
    if market in ("hk", "us", "crypto"):
        df = regime_builder.load_regime_market_history(_data_dir(request), market)
    else:
...
@router.get("/coverage")
def regime_coverage(request: Request, market: str = Query("cn", description="cn|hk|us|crypto")):
...
def regime_recompute(request: Request, start: date | None = None, end: date | None = None,
    ...
    - market=hk|us|crypto: 重算港美股/crypto 市场环境（动量替代投机维度）。
