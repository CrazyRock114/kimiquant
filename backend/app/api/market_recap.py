...
async def analyze_market(request: Request, req: AnalyzeRequest):
    ...
    market=hk|us|crypto（多市场扩展）: 走港美股复盘(新高/动量替代涨停/连板)。
...
def market_data_recap(request: Request, market: str = Query("hk", description="hk|us|crypto")):
    """港美股/crypto 数据版复盘（多市场扩展）— 不依赖 AI 接口。

    基于 build_market_overview_market 的结构化数据，直接生成 Markdown 复盘文本。
    AI 复盘（POST /analyze）需配置 AI 接口且暂仅支持 A 股。
    """
    if market not in ("hk", "us", "crypto"):
        raise HTTPException(status_code=400, detail="market 必须为 hk|us|crypto")
    ...
    lines = [
        ...
        "",
        "### 强度",
        f"- 60日新高 {l.get('limit_up', 0)} 只 / 60日新低 {l.get('limit_down', 0)} 只（该市场无涨跌停概念，以新高新低替代）",
    ...
    for r in (ov.get("top_losers") or [])[:5]:
        lines.append(f"- {r.get('symbol')} {r.get('name') or ''} {r.get('change_pct', 0) * 100:+.2f}%")
    src_label = "币安公开日K" if market == "crypto" else "TickFlow 免费日K"
    lines += ["", f"> 数据来源：本地 enriched 表（{src_label}）。此复盘为数据模板，AI 深度解读需配置 AI 接口。"]
...
@router.get("/reports")
def list_reports(request: Request, market: str = Query("cn", description="cn|hk|us|crypto")):
