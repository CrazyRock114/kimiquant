"""港美股/crypto 主要指数 API（多市场扩展）。

数据来自腾讯免费接口（TickFlow 免费模式无港美股指数）与币安现货日K（crypto），由
...
@router.get("/list")
def market_indices_list(request: Request, market: str = Query("hk", description="hk|us|crypto")):
...
@router.get("/quotes")
def market_indices_quotes(request: Request, market: str = Query("hk", description="hk|us|crypto")):
...
@router.post("/sync")
def market_indices_sync(request: Request, market: str = Query("hk", description="hk|us|crypto")):
