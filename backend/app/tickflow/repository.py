...
class DataStore:
    ...
    def __init__(self, data_dir: Path | None = None) -> None:
        ...
        for sub in (
            ...
            "kline_daily_hk",
            "kline_daily_enriched_hk",
            "kline_daily_us",
            "kline_daily_enriched_us",
            "kline_daily_crypto",
            "kline_daily_enriched_crypto",
            "kline_index_daily",
            "kline_index_enriched",
            "kline_index_daily_hk",
            "kline_index_daily_us",
            "kline_index_daily_crypto",
            "instruments_index_hk",
            "instruments_index_us",
            "instruments_index_crypto",
            "kline_etf_daily",
            "kline_etf_enriched",
            "kline_etf_minute",
            "kline_minute",
    ...
    def _register_views(self) -> None:
        ...
        statements = [
            ...
            f"""CREATE OR REPLACE VIEW kline_daily_crypto AS
                SELECT * FROM read_parquet('{d}/kline_daily_crypto/**/*.parquet', union_by_name=true)""",
            f"""CREATE OR REPLACE VIEW kline_enriched_crypto AS
                SELECT * FROM read_parquet('{d}/kline_daily_enriched_crypto/**/*.parquet', union_by_name=true)""",
...
class KlineRepository:
    ...
    def __init__(self, store: DataStore) -> None:
        ...
        # 港美股/crypto 独立 enriched 存储（多市场扩展）
        ...
        self._crypto_enriched_glob = str(store.data_dir / "kline_daily_enriched_crypto" / "**" / "*.parquet")

    # ── 多市场辅助（cn/hk/us/crypto）──────────────────────────────────
    def _enriched_glob_for_market(self, market: str) -> str:
        ...
        if market == "crypto":
            return self._crypto_enriched_glob
    ...
    def get_instruments_asset(self, asset_type: str, market: str = "cn") -> pl.DataFrame:
        ...
        market 参数（cn/hk/us/crypto）：instruments parquet 含 market 列（多市场扩展），
        A 股调用方不传时保持全量（兼容旧行为），港美股/crypto 按市场过滤。
        """
        if asset_type == "stock":
            df = self.get_instruments()
            if market in ("hk", "us", "crypto") and not df.is_empty() and "market" in df.columns:
