...
def run_market_sync(
    ...
    """港美股/crypto 数据同步 + enriched 计算（多市场扩展）。

    流程: instruments(market 过滤) → 日K(前复权批量拉取; crypto 走币安插件) → enriched 计算。
    A 股走 run_now；本函数服务港美股与 crypto，供 API/脚本按需触发。
    ...
    if market == "crypto":
        # crypto 标的维表走币安插件（合并写入，不覆盖其他市场），支持独立首次同步。
        # 失败只记日志继续用旧维表（fail-closed，universe 为空时下方会明确提示）。
        try:
            n = instrument_sync.sync_crypto_instruments(repo.store.data_dir)
            if n:
                emit("sync_instruments", 5, f"crypto 标的维表同步 {n} 只")
                _invalidate("instruments")
                repo.clear_cache()
                repo.refresh_cache()
        except Exception as e:  # noqa: BLE001
    ...
    # Step 1: 日K（港美股: forward 前复权，无除权因子表；crypto: 币安插件原始价，无复权概念）
    end = _dt.now()
    ...
    if market == "crypto":
        # crypto 不走 TickFlow：币安公开 klines 接口（plugins/binance）
