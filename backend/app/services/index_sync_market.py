"""港美股/crypto 主要指数同步（多市场扩展）。

TickFlow 免费模式无港美股指数，改用腾讯免费接口拉取主要指数日K：
  - 港股: 恒生指数 / 恒生科技 / 恒生国企
  - 美股: 标普500 / 纳斯达克 / 道琼斯 / 罗素2000
  - crypto: 复用币安插件拉 BTC/ETH 等主流币日K（无独立指数，以主流币代替）

存储: kline_index_daily_{hk,us,crypto}/date=*/part.parquet（与 A 股指数同构，symbol/date/OHLCV）
维表: instruments_index_{hk,us,crypto}/instruments.parquet
"""
...
#         binance=币安现货日K(crypto, 源code 为交易对如 BTCUSDT)
MARKET_INDICES: dict[str, list[tuple[str, str, str, str]]] = {
    ...
    "crypto": [
        ("BTC", "比特币", "binance", "BTCUSDT"),
        ("ETH", "以太坊", "binance", "ETHUSDT"),
        ("SOL", "索拉纳", "binance", "SOLUSDT"),
        ("BNB", "币安币", "binance", "BNBUSDT"),
        ("XRP", "瑞波币", "binance", "XRPUSDT"),
        ("DOGE", "狗狗币", "binance", "DOGEUSDT"),
    ],
}
...
def _fetch_binance_daily(code: str, days: int) -> pl.DataFrame:
    """币安现货日K（crypto 指数）。code 为交易对（BTCUSDT），内部映射为项目符号 BTCUSDT.CRYPTO。

    复用币安插件而不是裸 REST：限速/重试/代理口径与标的日K同步保持一致。
    返回列 [symbol, date(pl.Date), open, high, low, close, volume, amount]，
    symbol 由 sync_market_indices 统一覆写为维表 symbol（BTC 等），无需映射回。
    """
    from app.plugins.binance.provider import BinanceProvider

    provider = BinanceProvider()
    try:
        end = datetime.now(UTC)
        df = provider.get_daily([f"{code}.CRYPTO"], end - timedelta(days=days), end)
    finally:
        provider.close()
    return df


_FETCHERS = {"tencent": _fetch_tencent_daily, "tencent_new": _fetch_tencent_new_daily, "eastmoney": _fetch_eastmoney_daily, "realtime": _fetch_realtime, "binance": _fetch_binance_daily}


def sync_market_indices(market: str, data_dir: Path, days: int = 400) -> int:
    """同步港美股/crypto 主要指数日K + 维表。返回写入行数。
