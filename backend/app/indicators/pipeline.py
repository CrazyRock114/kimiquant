...
def compute_all(
    ...
    market: cn 走完整链路（含涨跌停/连板信号）；hk/us/crypto 无涨跌停概念，
...
def run_pipeline_market(market: str, data_dir: Path | None = None,
    ...
    """港美股/crypto 盘后管道：读 kline_daily_{market} → 计算 enriched → 写 kline_daily_enriched_{market}。

    港美股/crypto 差异:
      - 日K直接以入库价为准（港美股 forward 前复权 / crypto 无复权概念），compute_enriched 不传 factors。
      - 无涨跌停/连板概念，compute_enriched 不传 instruments（连板列缺省，下方补 0）。
    ...
    if market in ("cn",):
        raise ValueError("run_pipeline_market 仅用于港美股/crypto (hk/us/crypto)；A 股走 run_pipeline")
