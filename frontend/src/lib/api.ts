...
export const api = {
  ...
  indicesMarketQuotes: (market: 'hk' | 'us' | 'crypto') =>
    request<{ rows: IndexQuote[]; count: number }>(`/api/indices/market/quotes?market=${market}`),
  indicesMarketDaily: (market: 'hk' | 'us' | 'crypto', symbol: string, days = 180) =>
  ...
  indicesMarketMinute: (market: 'hk' | 'us' | 'crypto', symbol: string) =>
  ...
  pipelineRunMarket: (market: 'hk' | 'us' | 'crypto') =>
