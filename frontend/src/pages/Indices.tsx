...
// 港美股/加密置顶指数（多市场扩展）
const MARKET_PINNED: Record<'hk' | 'us' | 'crypto', { symbol: string; name: string }[]> = {
  hk: [
...
export function Indices() {
  ...
  const list = useQuery({
    ...
    queryFn: () => isCn ? api.indexList() : api.indicesMarketList(market as 'hk' | 'us' | 'crypto'),
  })
  ...
  const topRows = useMemo(() => {
    ...
    const pinned = isCn ? PINNED_INDEXES : MARKET_PINNED[market as 'hk' | 'us' | 'crypto']
    return pinned.map(p => (
  ...
  const listRows = useMemo(() => {
    ...
    const pinnedSymbols = new Set(MARKET_PINNED[market as 'hk' | 'us' | 'crypto'].map(p => p.symbol))
  ...
  const quotes = useQuery({
    ...
    queryFn: () => isCn ? api.indexQuotes() : api.indicesMarketQuotes(market as 'hk' | 'us' | 'crypto'),
  ...
  const daily = useQuery({
    ...
    queryFn: () => (isCn
      ...
      : api.indicesMarketDaily(market as 'hk' | 'us' | 'crypto', selectedSymbol, 180)) as any,
  ...
  const minute = useQuery({
    ...
    queryFn: () => (isCn
      ...
      : api.indicesMarketMinute(market as 'hk' | 'us' | 'crypto', selectedSymbol)) as any,
  ...
  const syncDaily = useMutation({
    mutationFn: () => (isCn ? api.syncIndexDaily(365) : api.indicesMarketSync(market as 'hk' | 'us' | 'crypto')) as any,
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: QK.indexList })
      qc.invalidateQueries({ queryKey: QK.indexQuotes })
